...
### network.rs (~280 lines)

Handles all mDNS operations with advanced obfuscation techniques.
...
The system maintains a database of 20+ realistic printer models from major vendors:

- HP (LaserJet Pro, OfficeJet, Color LaserJet, PageWide)
- Canon (imageRUNNER, LBP series)
- Xerox (VersaLink, WorkCentre)
- Brother (HL, MFC series)
- Ricoh (MP, AFICIO)
- Kyocera (ECOSYS, TASKalfa)
- Lexmark (MS, CX series)
- Toshiba (e-STUDIO)

Each profile includes randomized:

- Vendor and model name
- Instance name (printer identifier)
- Host name
- Port configuration
- Location metadata
...
25+ realistic office locations for metadata obfuscation:

- Department names (HR, Finance, Legal, Engineering)
- Room numbers (201-401)
- Floor designations (Floor 2-4)
...
The service randomly selects from multiple printer profiles each time a message is sent, making it difficult to fingerprint the covert channel through pattern analysis. Traffic appears as legitimate office printer announcements from different vendors and locations.
